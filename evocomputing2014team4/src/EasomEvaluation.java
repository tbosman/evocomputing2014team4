...
public class EasomEvaluation implements ContestEvaluation 
{
	// Evaluations budget
	...
	private final static double BASE_ = 0.99981431;// E[] =  -1.8568761020435928E-5
	// The minimum of the sphere function
	private final static double ftarget_= -1; // At ~ {+-1.4704758} ^10
	...
	{	
		...
		for(int i=0; i<10;i++) {
			cosX *= Math.cos(x[i]);
			piX -= (x[i]-Math.PI)*(x[i]-Math.PI);
		}
		
		out = cosX*Math.exp(piX);
